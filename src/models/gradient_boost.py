...
# HistGradientBoostingRegressor handles NaN natively — no imputation needed for lag12.
FEATURE_COLS = [
...
def _build_features(group: pd.DataFrame) -> pd.DataFrame:
    ...
    data["lag12"] = data["count"].shift(12)   # NaN intentionally left — HGBR handles it
    return data


def gradient_boost_forecast(monthly: pd.DataFrame) -> pd.DataFrame:
    if monthly.empty:
        return pd.DataFrame()
    ...
    for cat, grp in monthly.groupby("category"):
        ...
        feat = _build_features(grp).iloc[-1]
        if feat[core].isna().any():
            pred = float(grp["count"].tail(3).mean()) if len(grp) >= 3 else float(grp["count"].iloc[-1])
        else:
            pred = float(model.predict(feat[FEATURE_COLS].to_numpy(dtype=float).reshape(1, -1))[0])
